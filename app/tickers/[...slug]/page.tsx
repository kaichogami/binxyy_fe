...
export async function generateStaticParams() {
  const tickerData = await fetchTickersFromAPI();

  const allParams = tickerData.flatMap(({ ticker, name, description }) =>
    views.map(view => ({
      ticker,
      name,
      description,
...
export default async function Page({ params }: { params: { ticker: string; view: string, name: string, description: string, slug: string } }) {
  const { ticker, view } = params;

  const tickerData = fetchDataFromAPI(ticker, view)
